 ...
 *
 * 2.3 Message
 * Client <---> Server、Server <---> Server への通信は本質的に非同期です。
 ...
 * 空のメッセージは黙って無視されるので、余計な問題を起こさずにメッセージ間でCR-LFシーケンスを使用することができます。
 *
 ...
 *
 * 2) NUL(%x00)文字はメッセージ・フレーミングでは特別な文字ではなく、基本的にパラメーターの中に入れることができますが、通常のCの文字列処理では余計な複雑さを引き起こすことになります。したがって、NUL はメッセージの中では使えません。
 *
 *
 *
 *
